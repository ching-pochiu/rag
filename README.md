...
| 生成後查核 | 無：檢索候選直接組成 context 餵給 Gemini，答案內容沒有事後驗證機制 | Groundedness 檢查 + 引用頁碼反查 |
...
也就是說，這不是「重做一個類似的東西」，也不是「原模組很陽春、這個專題全面碾壓」——兩邊都有向量檢索跟 reranker，這部分是站在原模組的基礎上做的。真正的差異在於把「檢索到就相信」的單軌設計，換成切割更貼合文件結構、檢索管道更冗餘（向量+關鍵字雙軌）、生成後多一層事實查核的架構，並用系統性測試（見下方「成效驗證」）逐一驗證每一層真的有解決問題。
