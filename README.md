...
這個專題最初源自筆者在團隊專題 **[ConSpec-Agent](https://github.com/ching-pochiu/conspec-agent)**（工程品質審查系統）中負責的「CNS 法規問答」模組。回頭對照兩邊實際的程式碼後，發現原模組其實已經有 Cross-Encoder reranker 把關排序品質（甚至用了比這個專題更大的模型），但在另外幾個地方仍有明確可以強化的空間：切割時對表格與條文結構視而不見、檢索只有向量一條軌、答案生成後完全沒有事實查核。因此把這部分獨立拉出來重新設計，把切割方式、檢索管道、生成後查核這三層都補上結構化與驗證機制：

| | ConSpec-Agent 原模組 | 這個獨立專題 |
|---|---|---|
| **生成後查核** | **無：檢索候選直接組成 context 餵給 Gemini，答案內容沒有事後驗證機制** | **Groundedness 檢查 + 引用頁碼反查——兩邊都有 reranker 的情況下，這是唯一「連自己生成的答案都會回頭查」的一層** |
| 切割方式 | 兩軌並行：①23 筆由 Gemini 一次性協助抽取的精選條文 JSON（數量固定，非通用規則）；②全文 OCR 文字依頁切分後，用「累加至 900 字上限」的貪婪合併演算法切塊，只在行邊界斷開，對表格、節號、子項等結構完全無感知 | 條文依節號/子項邊界切割；表格另外偵測欄位結構，依牌號/代號分組逐塊切割——都是通用規則，可套用到任何新上傳的 PDF，不需要針對特定條文另外人工整理 |
| 檢索方式 | 純向量檢索（Ollama `qwen3-embedding:4b` + ChromaDB cosine 相似度），兩個 collection 分別查詢後直接合併候選池，交給 reranker 統一排序，沒有關鍵字比對這條軌 | 向量 + BM25 + RRF 融合排序：兩種訊號各自產生排名後用 RRF 加權合併，向量沒撈到、但關鍵字命中極強的內容也有機會進候選池 |
| 排序品質把關 | 有：Cross-Encoder Reranker（`bge-reranker-v2-m3`，5.68 億參數） | 有：Cross-Encoder Reranker（`bge-reranker-base`，2.78 億參數；受限於本機記憶體，是準確度與可運行性之間的取捨） |
| 表格資料處理 | 全文 OCR 文字裡的表格未經任何結構化處理，隨頁面文字一起被貪婪字數切塊 | 用 pdfplumber 偵測表格結構、合併多列表頭、去重複列，清理成乾淨表格後再依代號分組 |
| 關鍵數值查詢 | 完全依賴 LLM 根據檢索到的 context 生成答案，沒有繞過 LLM 的查表路徑 | 額外提供規則式查表（SD/SR/D 代號直接對照表格索引），跟 AI 答案雙軌互相驗證 |

也就是說，這不是「重做一個類似的東西」，也不是「原模組很陽春、這個專題全面碾壓」——兩邊都有向量檢索跟 reranker，這部分是站在原模組的基礎上做的。真正的差異在於把「檢索到就相信」的單軌設計，換成切割更貼合文件結構、檢索管道更冗餘（向量+關鍵字雙軌）、生成後多一層事實查核的架構。以上差異的實際效果，見下方「成效驗證」的 BM25 分詞、清單型查詢、Groundedness 三個實測案例——不是空談架構，每一項差異都有對應的具體數字佐證。
...
| 混合檢索 + RRF 融合排序 | 向量語意搜尋 + BM25 關鍵字搜尋，依兩路排名加權合併（Reciprocal Rank Fusion），並用「向量核可清單」擋掉 BM25 純字面命中的雜訊 | 語意相近與精確關鍵字命中互補，比單一方法或簡單聯集更可靠 |
...
中文 BM25 逐字分詞、生成 Prompt 措辭校準、表格依代號分組切塊、Cross-Encoder Reranker、Groundedness 檢查這五項，下方「成效驗證」有實測前後對照，這裡不重複列出。
