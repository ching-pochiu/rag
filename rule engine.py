...
# 英數字連續當一個 token（涵蓋「SD420W」這種代號），中文沒有空格分詞，
# 一律拆成單字當 token（見下方 _bm25_preprocess 說明）。
_BM25_TOKEN_RE = re.compile(r"[A-Za-z0-9]+|[一-鿿]")


def _bm25_preprocess(text: str) -> list:
    """
    BM25 預設分詞（langchain 的 default_preprocessing_func）就是單純的
    `text.split()`——只依空白字元切詞。這對中文完全失效：中文詞語間本來
    就沒有空格，一句「SD420W和SD690的抗拉強度和降伏強度」整句會被當成
    「唯一一個 token」，跟語料庫裡任何文件都對不上（語料庫文字也是連續
    中文，同樣會被切成各自獨立的長 token，兩邊幾乎不可能字元完全相同）。
    BM25 對所有文件分數就會是 0——這時候 rank_bm25 的 get_top_n 並不會
    回傳「沒有結果」，而是把同分（都是 0 分）的文件按語料庫原始順序排序
    後傳回，等於是看起來有分數、實際上是隨機雜訊的結果，卻會頂著這個
    分數混進 RRF 融合排序，稀釋掉真正該出現的候選（實測發現這種混合
    中文/代號的查詢，BM25 前幾名幾乎全是跟問題完全無關的內容）。

    做法分兩步：
    1. 先用正則把「字母代號 + 空白 + 數字」這種格式的空白拿掉（例如
       PDF 抽出來的「SD 420」變成「SD420」），語料庫跟查詢兩邊都套用，
       讓代號兩邊能對齊，不管原文有沒有這個空白。
    2. 分詞不再依賴空白字元，改用正則抓「連續英數字」當一個 token
       （例如「SD420W」），中文字沒有天然詞界，退而求其次逐字拆開當
       token（例如「抗拉強度」拆成「抗」「拉」「強」「度」四個 token）。
       這不是正確的中文斷詞，但對 BM25 這種字面重疊比對來說已經足夠：
       只要查詢跟語料庫共用的中文字夠多，就能算出有意義的相關性分數，
       不會再落入「整句當一個 token、全部兜不上」的全零分陷阱。
    這個函式在建語料庫索引、跟查詢時分詞都會用到（BM25Retriever 內部
    對兩者用同一個 preprocess_func），所以兩邊會一致正規化。
    """
    normalized = _BM25_CODE_MERGE_RE.sub(r"\1\2", text)
    return _BM25_TOKEN_RE.findall(normalized)
...
if uploaded_files:
    ...
    if st.button("送出查詢") and user_query:
        with st.spinner("正在透過 Gemini 進行檢索與總結..."):
            ...
            system_prompt = (
                "你是一位專業且嚴謹的土木規範專家。\n"
                "請根據【參考文本】回答問題，不管答案是數值、規定內容或處理程序都要"
                "直接具體回答，並嚴格根據文本中標註的【第 X 頁】與表格編號進行引用說明。\n"
                "若【參考文本】中確實找不到與問題相關的規定內容，"
                "請直接回答「查無相關規定，請人工確認」，不要自行推測或編造內容。\n\n"
                "【參考文本】：\n{context}\n\n"
                "【問題】：\n{question}"
            )
